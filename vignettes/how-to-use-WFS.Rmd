...
With the `startindex` and `maxFeatures` (1.1.0) or `count` (2.0.0) argument a subset of a query is produced. The `WFS__getfeature` function will use the correct version. By adding the `echo_request=T` clause the generated request url will be displayed so that you can verify this: `count` is translated to `maxFeatures` . Notice that `startIndex=2` indicates that the first feature to be retrieved is number 3. In other words `startIndex` indicates the number of records to skip.
...
- For two or more arguments `filter` has to be used in combination with 'POST'
- `filter` worked as expected for WFS version '2.0.0' 
- `filter` only works for '1.1.0' when the PropertyName 'boom_omschrijf' is generated in stead of 'topp:boom_omschrijf'
...
xml_query2=build_filter(version='1.1.0'
           , propeq_xml('topp:boom_omschrijf',"Alnus glutinosa 'Laciniata'") 
  )

wfs8d      <-  WFS_getfeature(typename,filter=xml_query2  # dit werkt niet
                              ,sortby='jaar ASC,objec_omschrijf DESC'
                              ,httrType = 'POST'
                              ,count=100
                              ,propertyname=fields
...
xml_query3=build_filter(version='1.1.0'
           , propeq_xml('boom_omschrijf',"Alnus glutinosa 'Laciniata'") 
  )

wfs8e      <-  WFS_getfeature(typename,filter=xml_query3  # dit werkt wel
                              ,sortby='jaar ASC,objec_omschrijf DESC'
                              ,httrType = 'POST'
                              ,count=100
                              ,propertyname=fields
