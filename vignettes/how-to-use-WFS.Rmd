...
The package contains some tools to build spatial queries that are described in the section [Spatial Tools](#spatial-tools) and this vignette ends with some [References](#references) on the internet.
...
In this section we describe some tools (functions) that can be useful when formulating spatial queries. Even when it is often possible to download the whole data set and filter the resulting `sf` object with [dplyr](https://cran.r-project.org/package=dplyr) (in case of a query of the attributes) or [sf](https://cran.r-project.org/package=sf) (in case of a query on the geometry) it is more efficient to let this be handled by the WFS server. 
